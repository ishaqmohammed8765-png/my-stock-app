"""
Stock Calculator Pro — Personal Trading Dashboard
Clean, intuitive statistical trading app for personal use
"""
...
tab1, tab2, tab3 = st.tabs(["💰 Trade", "📊 Chart", "📁 Portfolio"])
...
with tab1:
    ...
    if c1.button("Scalp (5d)"):
        st.session_state.horizon = 5
    if c2.button("Swing (30d)"):
        st.session_state.horizon = 30
    if c3.button("Position (90d)"):
    ...
    with st.form("trade"):
        c1, c2 = st.columns(2)
        with c1:
            buy = st.number_input("Buy", value=st.session_state.buy_price)
            qty = st.number_input("Quantity", min_value=1, value=st.session_state.qty)
        with c2:
            sell = st.number_input("Sell", value=st.session_state.sell_price)
    ...
    if submit:
        ...
        exp_move = S * σ * np.sqrt(T)

        c1, c2 = st.columns(2)
        c1.metric("Buy Probability", f"{buy_p:.1%}")
        c1.caption(interpret(buy_p))
        c2.metric("Sell Probability", f"{sell_p:.1%}")
        c2.caption(interpret(sell_p))
        ...
        if abs(buy - S) > 2 * exp_move:
            st.warning("Buy target > 2σ away")
        if abs(sell - S) > 2 * exp_move:
...
with tab2:
    ...
    fig.update_layout(
        height=650,
        title=f"{st.session_state.ticker} — Expected Move View",
