...
def exit_fill(bar: pd.Series, stop: float, target: float, cfg: Execution, *, due=False, at_open=True, entry_day=False):
    if due:
    ...
    ambiguous = bool((stop_hit and target_hit) or (entry_day and not at_open and (stop_hit or target_hit)))
    if stop_hit:
...
def position_size(cash: float, entry: float, stop: float, cfg: Execution) -> int:
    ...
    return max(0, min(affordable, math.floor(risk_budget/risk_per_share))) if risk_per_share > 0 else 0
...
def summarise(equity: pd.Series, trades: pd.DataFrame) -> dict:
    ...
    return {
        "trades": len(trades), "total_return": float(equity.iloc[-1]/equity.iloc[0]-1),
        "net_pnl": float(equity.iloc[-1]-equity.iloc[0]),
        "max_drawdown": float((equity/equity.cummax()-1).min()),
        "sharpe": float(returns.mean()/sd*np.sqrt(252)) if len(returns) > 1 and sd > 0 else np.nan,
...
def run_backtest(df: pd.DataFrame, strategy: Strategy, execution: Execution, *, start=200, end=None) -> Backtest:
    ...
    def close_position(price, reason, j):
        ...
        records.append({"entry_date": df.index[p["entry_index"]], "exit_date": df.index[j],
            "entry_price": p["entry_price"], "exit_price": price, "stop": p["stop"], "target": p["target"],
            "quantity": p["quantity"], "sessions_held": j-p["entry_index"], "reason": reason,
            "gross_pnl": gross, "fees": 2*execution.commission, "net_pnl": net, "net_r": net/p["planned_risk"]})
        position = None

    for j in range(start, end):
        bar, exited = df.iloc[j], False
        if position is not None:
            price, reason, ambiguous = exit_fill(bar, position["stop"], position["target"], execution, due=j-position["entry_index"] >= strategy.horizon)
        ...
        if position is None and not exited and j < end-1:
            ...
            if setup.eligible:
                ...
                if fill is not None:
                    entry, at_open = fill
                    stop, target = entry-strategy.atr_stop*previous.atr14, entry+strategy.atr_target*previous.atr14
                    ...
                    if qty == 0:
                        ...
                        position = {"entry_index": j, "entry_price": entry, "stop": stop, "target": target, "quantity": qty,
                            "planned_risk": (entry-market_fill(stop, "sell", execution))*qty+2*execution.commission}
                        price, reason, ambiguous = exit_fill(bar, stop, target, execution, at_open=at_open, entry_day=True)
                        diagnostics["ambiguous_bars"] += int(ambiguous)
                        if price is not None:
                            close_position(price, reason, j)
        ...
        values.append(float(cash+(position["quantity"]*bar.close if position is not None else 0)))
        dates.append(df.index[j])
    equity = pd.Series(values, index=pd.DatetimeIndex(dates), name="Strategy")
    columns = ["entry_date", "exit_date", "entry_price", "exit_price", "stop", "target", "quantity", "sessions_held", "reason", "gross_pnl", "fees", "net_pnl", "net_r"]
...
def buy_and_hold(df: pd.DataFrame, cfg: Execution, start: int, end: int) -> pd.Series:
    ...
    values[-1] = cash+qty*market_fill(float(df.close.iloc[end-1]), "sell", cfg)-cfg.commission
